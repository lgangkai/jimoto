...
import (
	...
	"gorm.io/gorm"
	"jimoto/account-service/conf"
	"jimoto/account-service/dao"
	"jimoto/account-service/wire"
	"log"
	"protos/account"
)
...
func (s *Server) Init() error {
	...
	// 1. load config file.
	var confPath string
	flag.StringVar(&confPath, "config", "conf/account_live.yaml", "define config file")
	flag.Parse()
	...
	// 4. injection.
	accountHandler := wire.InitAccountHandler(
	...
	// 5. init service
	s.service.Init()
	err = account.RegisterAccountHandler(s.service.Server(), accountHandler)
	if err != nil {
		log.Println("register AccountHandler failed, err: ", err.Error())
		return err
	}
